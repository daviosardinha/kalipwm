...
Usage:
  ./kalipwm.sh                 Interactive install
  ./kalipwm.sh --rollback      Restore the latest pre-install backup
  ./kalipwm.sh --uninstall     Remove KaliPWM V1 and restore latest backup
  ./kalipwm.sh --help          Show this help
EOF
}
...
install_packages() {
  local common guest=() available=() pkg
  common=(
...
install_shell_stack() {
  ...
  if [[ "${SHELL:-}" != "$(command -v zsh)" ]] && confirm "Make ZSH your default shell?" Y; then
...
install_configs() {
  ...
  # Preserve compatibility with the upstream target command without writing into /usr/bin.
  if [[ -f "$HOME/.config/polybar/forest/scripts/target.sh" ]]; then
    ln -sfn "$HOME/.config/polybar/forest/scripts/target.sh" "$LOCAL_BIN/target"
...
write_profile() {
  ...
  if [[ "$ENVIRONMENT" == baremetal ]] && confirm "Place external displays to the right of the laptop by default?" Y; then
    EXTERNAL_POSITION=right
  elif [[ "$ENVIRONMENT" == baremetal ]]; then
...
External position : ${EXTERNAL_POSITION:-right}
Editor            : Vim
...
main() {
  case "${1:-}" in
    --help|-h) usage; exit 0 ;;
    --rollback) require_normal_user; restore_backup; exit 0 ;;
    --uninstall) require_normal_user; restore_backup; exit 0 ;;
    '') ;;
  ...
  # Re-write after install to ensure profile survives copied defaults.
  write_profile

  # Existing sessions can pick up the new bindings without a reboot.
  pkill -USR1 -x sxhkd 2>/dev/null || true
  "$HOME/.config/polybar/forest/launch.sh" >/dev/null 2>&1 || true
